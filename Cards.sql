--DROP TABLE "Cards";
--DROP TABLE "FindInShops";
--DROP FUNCTION create_link() CASCADE;
...
(
    ...
    "Original set price" money NOT NULL,
	"Current discount" smallint CHECK("Current discount" BETWEEN 0 AND 100),
	"Current set price" money NOT NULL,
...
INSERT INTO "FindInShops" ("id", "Set Name", "Shop Name", "Original set price", "Current discount")
VALUES (001, 'Ikoria', 'Hobby Games', 2100, 5),
		(002, 'Core21', 'Hobby Games', 1990, 0),
		(003, 'Ikoria', 'Citadel', 2080, 3),
		(004, 'Kaldheim', '20Graney', 2000, 3),
		(005, 'Strixhaven', 'Hobby Games', 1990, 2),
		(006, 'Ikoria', '20Graney', 2200, 10),
		(007, 'Core21', 'Citadel', 1900, 0);
		
SELECT * FROM "FindInShops";

CREATE OR REPLACE FUNCTION answer_from_parser(IN url character varying(100), OUT url0 character varying(100))
LANGUAGE plpgsql AS $$
BEGIN
END;$$;

CREATE OR REPLACE FUNCTION count_discount()
RETURNS TRIGGER AS $$
BEGIN
	NEW."Current set price" = OLD."Original set price" - OLD."Current discount" * OLD."Original set price" / 100;
   	RETURN NEW;
END;
$$ language 'plpgsql';

DROP TRIGGER IF EXISTS added_card on "Cards";

CREATE TRIGGER added_card AFTER INSERT
